...
def search_bar(text_pole, combo_pole):
    ...
    x = list[combo_pole]
    list2 = [Zarizeni.zar_inv==text_pole, Zarizeni.zar_seriove==text_pole, Transakce.tran_poznm==text_pole, Uzivatel.uziv_kod==text_pole, Uzivatel.uziv_email==text_pole]
    xx = list2[combo_pole]
    try:
...
def device_listing(text_pole=None, combo_pole=0):
    ...
    x = list[combo_pole]
    dotaz3 = (db_session.query(Zarizeni, Kategorie, Vyrobce, Transakce)
            .join(Kategorie, Zarizeni.id_kat == Kategorie.id_kat)
            .join(Vyrobce, Zarizeni.id_vyr == Vyrobce.id_vyr)
            .join(Transakce, Zarizeni.id_zar == Transakce.id_zar)
            .order_by(desc(Zarizeni.id_zar))
            .where(and_(x, Transakce.id_lok != 1, Transakce.tran_platnost_do.is_(None)))
            .limit(50)
            .all())
    db_session.close()
    return dotaz3
...
def transaction_listing(text_pole, combo_pole_t,y=50):
    if text_pole == None: # první načtení přehledu bez zadaných parametrů
        ...
        list = [Zarizeni.zar_inv==text_pole, 
                ...
                Transakce.tran_poznm==text_pole,
