...
class Vyrobce(Base):
    ...
    vyr_nazev: Mapped[str] = mapped_column(String(32), unique=True) # název výrobce
...
class Kategorie(Base):
    ...
    kat_nazev: Mapped[str] = mapped_column(String(32), unique=True) # název kategorie - pc, ntbk, workst,...
...
class Status(Base):
    ...
    stat_nazev: Mapped[str] = mapped_column(String(32), unique=True) # název statusu
...
class Zarizeni(Base):
    ...
    zar_seriove: Mapped[str] = mapped_column(String(32), unique=True) # sériové číslo
    zar_model: Mapped[str] = mapped_column(String(32)) # model zařízení
    ...
    id_kat: Mapped[int] = mapped_column(ForeignKey("Kategorie_tab.id_kat"))
    id_vyr: Mapped[int] = mapped_column(ForeignKey("Vyrobci_tab.id_vyr"))
...
class Uzivatel(Base):
    ...
    uziv_kod: Mapped[str] = mapped_column(String(32)) # id-čko
    uziv_jmeno: Mapped[str] = mapped_column(String(32)) # jméno
    uziv_prijmeni: Mapped[Optional[str]] = mapped_column(String(32)) # příjmené
    uziv_email: Mapped[Optional[str]] = mapped_column(String(64), unique=True) # email
    ...
    uziv_heslo: Mapped[Optional[str]] = mapped_column(String(64)) # heslo
    # FK - vztah, opravneni
    id_vzt: Mapped[int] = mapped_column(ForeignKey("Vztahy_tab.id_vzt"))
    id_opr: Mapped[int] = mapped_column(ForeignKey("Opravneni_tab.id_opr"))
...
class Vztah(Base):
    ...
    vzt_nazev: Mapped[str] = mapped_column(String(32)) # název vztahu - externi, interni, vyrobni, ot
...
class Opravneni(Base):
    ...
    opr_nazev: Mapped[str] = mapped_column(String(32)) # název oprávnění
...
class Lokace(Base):
    ...
    lok_kod: Mapped[str] = mapped_column(String(8), unique=True) # kód lokace
    lok_nazev: Mapped[str] = mapped_column(String(32)) # název lokace

    id_bud: Mapped[int] = mapped_column(ForeignKey("Budovy_tab.id_bud"))
    ...
    def __repr__(self):
        return f"<Lokace(id_lok={self.id_lok}, lok_kod='{self.lok_kod}', lok_nazev='{self.lok_nazev}', id_bud={self.id_bud})>"
...
class Budova(Base):
    ...
    bud_nazev: Mapped[str] = mapped_column(String(32)) # název budovy A-B-C
...
class Transakce(Base):
    ...
    tran_poznm: Mapped[str] = mapped_column(String(64)) # poznámka oprávnění
    tran_editace: Mapped[datetime.datetime] = mapped_column(DateTime)

    id_uziv: Mapped[int] = mapped_column(ForeignKey("Uzivatele_tab.id_uziv"))
    id_zar: Mapped[int] = mapped_column(ForeignKey("Zarizeni_tab.id_zar"))
    id_lok: Mapped[int] = mapped_column(ForeignKey("Lokace_tab.id_lok"))
    id_stat: Mapped[int] = mapped_column(ForeignKey("Statusy_tab.id_stat"))
    ...
    def __repr__(self):
        return f"<Transakce(id_tran={self.id_tran}, tran_platnost_od='{self.tran_platnost_od}', tran_platnost_do='{self.tran_platnost_do}', tran_poznm={self.tran_poznm}, tran_editace='{self.tran_editace}', id_uziv='{self.id_uziv}', id_zar={self.id_zar}, id_lok='{self.id_lok}', id_stat={self.id_stat})>"
